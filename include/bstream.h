...
namespace bStream {
	...
	bool CMemoryStream::Reserve_A(std::size_t needed) {
		if (mCapacity >= needed)
			return true;
		...
		std::size_t newCap = mCapacity;
		while (newCap < needed)
			newCap *= 2;

		uint8_t* temp = new uint8_t[newCap];
		if (mBuffer && mSize > 0)
			memcpy(temp, mBuffer, mSize);
	...
	void CMemoryStream::alignTo(std::size_t to) {
		std::size_t nextAligned = (static_cast<std::make_signed_t<std::size_t>>(-static_cast<std::make_signed_t<std::size_t>>(mPosition)) % to) % to;
